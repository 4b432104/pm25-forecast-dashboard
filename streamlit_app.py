...
def render_backtest_section():
    """過去 24 小時歷史追溯驗證 (Backtesting) 圖表區塊"""
    st.markdown("---")
...
def main():
    ...
    # 💡 解析 4 個門架獨立車流量與計算總流量
    # 特徵欄位順序: [11]: 03F2100N, [12]: 03F2100S, [13]: 03F2125N, [14]: 03F2129S
    ...
    # 自動推算下載的的 TDX 高速公路 M05A 車流 CSV 檔名格式
    # 格式通常為：TDX_M05A_YYYYMMDD_HH0000.csv (對應前一小時前之數據檔)
    downloaded_filename = f"TDX_M05A_{prev_hour.strftime('%Y%m%d_%H0000')}.csv"
    ...
    # ------------------------------------------------------------------
    # 📝 核心：工作日誌與「下載檔名 + 各門架車流量細節」
    # ------------------------------------------------------------------
    with st.expander("📝 查看車流量下載檔名、門架細節與工作日誌 (Work Log)", expanded=False):
        st.markdown(f"**📁 成功解析數據檔名**：`{downloaded_filename}`")
        st.markdown(f"**⏰ 資料時間點**：`{current_time_str}` | **車流統計時段**：`{traffic_range_str}`")
    ...
    # 7. 渲染「過去 24 小時歷史追溯驗證 (Backtesting)」區塊
